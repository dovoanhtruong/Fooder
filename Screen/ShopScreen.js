...
const[foodinshop,setFoodInShop]=useState([]);
...
useEffect(()=>{
    ...
    socket.on('server_msg', msg=>{
      fetch('http://10.0.2.2:3000/food/shop/'+idShop)
      .then((res)=>res.json())
      .then((data)=>{
        setFoodInShop(data.ListFood)
      })
  ...
  useEffect(()=>{
    fetch('http://10.0.2.2:3000/shop/shop/'+idShop)
    .then((res)=>res.json())
    .then((data)=>{
        setFoodInShop(data.ListFood)
        setDetailShop(data.DetailShop)
})
    .catch((err)=>console.error(err))
}, [])
...
return(
    ...
    <View >
        <View>
            ...
            source={{ uri: detailShop.imageshop}}
        />

            <Text style={styles.TextShop} >{detailShop.nameshop}</Text>
            <Text style={styles.TextShop} >{detailShop.addressshop}</Text>
